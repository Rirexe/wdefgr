...
client.on("guildMemberAdd", member => {
  ...
  const channel = sWlc[member.guild.id].channel
    ...
    const devs = ['467023557281447936'];

client.on('message', message => {
    let argresult = message.content.split(` `).slice(1).join(' ');
    if (message.content.startsWith(prefix + 'setStreaming')) {
      if (!devs.includes(message.author.id)) return message.channel.send("<@467023557281447936> only this guy can do restart the bot so don't try again :wink:.");
      message.delete();
...
client.login("NTM1NTM0NDg2NDc4MTI3MTA0.DycQmQ.DhuMN-BbAqWaEo1L4usNRh1t8Oc");
